...
var Feed = require('rss-to-json'); //fuck this feels so mufkin bloated
...
app.get('/ac', function(req,res) { // bascially no-cors but less bloated, that muhfucka is so bloated, you only need like 20 lines max to do this shit xdxdxd
    exec(`curl 'https://www.google.com/complete/search?q={{QUERY}}&cp=3&client=gws-wiz&xssi=t&gs_ri=gws-wiz&hl=en&authuser=0&psi=Y22hYJftKqPn5NoPxc2UuAc.1621192036071&dpr=2' \
    ...
    --compressed`.replace('{{QUERY}}',encodeURIComponent(req.query.q)), (error, stdout, stderr) => {
        try { 
